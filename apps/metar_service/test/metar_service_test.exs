defmodule MetarServiceTest do
  ...
  setup_all do
    ...
    {:ok, _} = MetarService.Coordinator.start_link
    {:ok, %{}}
  end
  ...
  @tag :skip
  test "it returns an error for a station without data" do
    assert MetarService.get("CYYZ") == {:error, "Report not available."}
  end

  @tag :skip
  test "it returns the metar for the requested reporting station" do
    # let's leave a failing acceptance test
    assert MetarService.get("CYVR") == {:ok,
      %{metar: %MetarService.Station{elevation_m: '2.0', flight_category: 'VFR',
       latest_observation_time: '2017-11-16T12:53:00Z', latitude: '49.17',
       longitude: '-123.17',
       reports: %{
         current:  'CYVR 161253Z VRB02KT 20SM FEW022 SCT063 BKN120 06/03 A2951 RMK SC2SC1AC4 SLP995',
         history: ['CYVR 161216Z 24010KT 180V240 15SM -RA BKN023 OVC039 06/04 A2951 RMK SC6SC2 WSHFT 1207 SLP995',
       ...
       sea_level_pressure_mb: '999.5', station: "CYVR"},
       nearby_stations: ["CYXX", "CYYF", "CYQQ", "CYXC", "CYVR", "CYHE", "CYYJ", "CYBC", "CYYY", "CYDF", "CYQX", "CYGP", "CYJT"],
       retrieved_at: "2017-12-12T01:01:13.479367Z",
